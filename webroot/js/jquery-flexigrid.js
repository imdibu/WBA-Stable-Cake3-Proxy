...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 showToggleBtn: false, //show or hide column toggle popup // Don't allow toggleCol
			 hideOnSubmit: true,
			 autoload: true,
			 blockOpacity: 0.5,
			 onToggleCol: false,
		...
		var g = {
						...
						var nw = this.colresize.nw;
								...
								$('tr',this.bDiv).each (
									function ()
										...
										// Change to handle RowSpan
										$('td[colInd='+n+'] div',this).css('width',nw);
										}
								);
			...
			toggleCol: function(cid,visible) {
				...
				// Don't hide Default Columns	
				if($.inArray(p.colModel[cid].name,p.defaultColumns)>-1){
					cb.checked = true;
					return false;
				}
				...
				else
					{
						...
						$('tbody tr',t).each
							(
								function ()
									{
										// Change to handle RowSpan - Begin
										/*
										if (visible)
											$('td:eq('+n+')',this).show();
										else
										...
										*/
										if (visible)
											$('td[colInd='+n+']',this).show();
										else
											$('td[colInd='+n+']',this).hide();
										// Change to handle RowSpan - End
									}
							);							
						...
						function ()
							{
									...
									// Change to handle RowSpan - Start
									//var n = $('td',$(this).parent()).index(this);
									if($(this).attr("colInd")){
										var n = $(this).attr("colInd");
									}else{
										var n = $('td',$(this).parent()).index(this);
									}
									// Change to handle RowSpan - End
			...
			pager: 0
			,recordColProp: function(){
				var colProp = new Array;
				$('thead tr:first th:visible',g.hDiv).each(function(){
					colProp.push($(this).attr("abbr")+":"+$("div",this).width());
				});
				
				if (p.onRecordColProp) p.onRecordColProp(colProp);
			}
		};		
